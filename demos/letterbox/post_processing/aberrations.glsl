...
vec3 tex2D(Image _tex, vec2 _p){
    vec3 col = Texel(_tex, _p).xyz;
    if (abs(_p.x - 0.5) > 0.5) {
        col = vec3(0.1);
    }
    return col;
...
// Love2d's effect function: vpos is the pixel coordinate, but we want normalized UVs
vec4 effect(vec4 color, Image tex, vec2 uv, vec2 screenpos)
{
    ...
    for(float x = -4.0; x < 2.5; x += 1.0){
        col += vec3(
            tex2D(tex, uvn + vec2(x - 0.0, 0.0) * 0.007).x,
            tex2D(tex, uvn + vec2(x - 2.0, 0.0) * 0.007).y,
            tex2D(tex, uvn + vec2(x - 4.0, 0.0) * 0.007).z
        ) * 0.1;
    }
    col *= 0.6;
